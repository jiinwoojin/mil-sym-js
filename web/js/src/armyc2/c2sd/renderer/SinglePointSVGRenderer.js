...
//                    _bufferDisplayModifiers = this.createBuffer(250,250);
	...
	//ctx.font="150pt UnitFontsC"; // * 4 (because font file is 25% of original)
        ...
        if(render === true)
        {
            ...
            {
/*              //20200109 Annotate to hide border color
            ...
            if (fill !== null &&  fillColor !== null) {
                ...
                tgPaths.push(fill);
                //20200219 multi color add. color1~
                if( basicID.substring(0,8) === ("G*G*GPDL") ) {
                    var arrMultiColor = [];
                    if (basicID === ("G*G*GPDLA-****X")              // 2.2.1.1.11.2.1
                            || basicID === ("G*G*GPDLW-****X")) {   // 2.2.1.1.11.2.5
                        arrMultiColor = ["#0000FF"];
                    } else if (basicID === ("G*G*GPDLC-****X")) {   // 2.2.1.1.11.2.2
                        arrMultiColor = ["#FFFF00"];
                    } else if (basicID === ("G*G*GPDLE-****X")        // 2.2.1.1.11.2.3
                            || basicID === ("G*G*GPDLH-****X")) {   // 2.2.1.1.11.2.4
                        arrMultiColor = ["#FF0000"];
                    } else if (basicID === ("G*G*GPDLO-****X")) {   // 2.2.1.1.11.2.7
                        arrMultiColor = ["#006400"];
                    } else if (basicID === ("G*G*GPDLJ-****X")) {   // 2.2.1.1.11.2.8
                        arrMultiColor = ["#FF0000", "#FFFF00", "#0000FF"];
                    }
                    for (var k = 0; k < arrMultiColor.length; k++) {
                        var tmpFill = null;
                        fillID = SymbolUtilities.getTGFillSymbolCodeN(symbolID, k+1);
                        if (fillID !== null) {
                            intFill = SinglePointLookup.getCharCodeFromSymbol(fillID,symStd);
                        }
                        if (intFill > 0) {
                            tmpFill = SPSVGTable.getSVGPath(intFill);
                        }
                        fillColor = arrMultiColor[k];
                        tmpFill = this.processSVGPath(tmpFill, fillColor,null,null,null,null,null,fillAlpha);
                        multiFill += tmpFill;
                    }
                }
            } else if ( fill !== null && (basicID === "G*G*ASG---****X" || basicID === "G*O*O-----****X" || basicID === "WAS-WP----P----" )) {
                if ( basicID === "G*G*ASG---****X" ) {
                    fillColor = "#666666";
                } else {
                    fillColor = "#FF0000";
                }
            ...
            //then draw frame
            if (frame !== null) {
                if ( basicID === "G*G*ASF---****X" ) {
                    fillColor = "#666666";
                    frame = this.processSVGPath(frame, fillColor,lineColor,"4",null,null,null,lineAlpha);
                } else {
        ...
        if(svgElements.length > 0 || svgElementsDOM.length > 0)
        {
            ...
            {
                returnSVG += svgElements[k] + "\n";
                var dom = new DOMParser().parseFromString(svgElements[k], 'text/html');
                var body = dom.body;
                var svg = '';
                if (body) {
                    var texts = body.children;
                    for (var i = 0; i < texts.length; i++) {
                        var text = texts[i];
                        text.setAttribute('fill-opacity', lineAlpha);
                        text.setAttribute('stroke-opacity', lineAlpha);
                        svg += text.outerHTML;
                    }
                    returnSVG += svg + '\n';
                }
...
//                x = symbolBounds.getCenterX() - centerOffset - (labelBounds1.getWidth()/2);
                ...
                basicID ===("G*G*APC---****X"))
        {
            if(modifiers.T)
            {
                ...
                var labelWidth = ti.getBounds().width;
        ...
        { //Targets with special modifier positions
            ...
            if(modifiers.H1 && basicID === ("G*F*PTS---****X"))//H1
            {
                ...
                //labelWidth = Math.round(ti.getTextBounds().getWidth());
                labelWidth = Math.round(ti.getBounds().width);
                x = bounds.getCenterX() - (bounds.width * 0.15);
                y = bounds.y + (bounds.height * 0.75);
                y = y + (labelHeight * 0.5);
        ...
        else if(basicID ===("G*G*GL5A--****X")) //20200219 add
        {
            if(modifiers.H)//H
            {
                strText = modifiers[ModifiersTG.H_ADDITIONAL_INFO_1];
            ...
            if(modifiers.T)//T
            {
                ...
                // points
                x = bounds.x + (bounds.width * 0.5);
                y = ((bounds.height * 0.50) - labelHeight);
                y = bounds.y + y;
        ...
        else if(basicID.charAt(0) === 'W')
        {
            ...
            else if(basicID ===("WOS-HDS---P----"))//Soundings
            {
                var fInfo1 = fontInfo;
                var fInfo2 = fontInfo;
                strText = "6";
                strText1 = "3";
                if( (modifiers.X !== undefined && modifiers.X1 !== undefined)
                 && !( modifiers.X === "" && modifiers.X1 === "") ) {
                    if(modifiers.X !== undefined)
                        strText = modifiers[ModifiersTG.X_ALTITUDE_DEPTH];
                    if(modifiers.X1 !== undefined)
                        strText1 = modifiers[ModifiersTG.X1_ALTITUDE_DEPTH];
                }

                fInfo1.size = "28";
                fInfo1.style = "normal";
                ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fInfo1,"middle", 0);  //textInfoContext : "bold 10pt Arial, sans-serif"
                labelWidth = Math.round(ti.getBounds().getWidth());
                //One modifier symbols and modifier goes in center
                x = bounds.x + (labelWidth * 1.2);
                y = bounds.y + (labelHeight * 2.0);
                ti.basicID = basicID;
                ti.setLocation(Math.round(x),Math.round(y));
                arrMods.push(ti);
                ...
                fInfo2.style = "normal";
                //labelWidth = Math.round(ti.getBounds().getWidth());
                ti = new SVGTextInfo(strText1,new armyc2.c2sd.renderer.so.Point(0,0),fInfo2,"middle", 0);  //textInfoContext : "bold 10pt Arial, sans-serif"
                //One modifier symbols and modifier goes in center
                x = x + (labelWidth * 1.5);
                y = y + (labelHeight * 1.1);
                ti.basicID = basicID;
            ...
            else if(basicID ===("WA--FI---------") || basicID ===("WA--FV---------"))//Instrument ceiling
            {
                strText = ""; //strText = "X?";
                if(modifiers.X !== undefined)
                    strText = modifiers[ModifiersTG.X_ALTITUDE_DEPTH];
            ...
            else if(basicID ===("WA-DWSTSWA--A--") )//Tropical storm wind areas and date/Time labels
            {
                strText = ""; //strText = "X?";
                if(modifiers.X !== undefined)
                    strText = modifiers[ModifiersTG.W_DTG_1];

                ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fontInfo,"start");
                labelWidth = Math.round(ti.getBounds().getWidth());
...

        {
            ...
            if(modifiers.H)//H
            {
                ...
                if (basicID===("G*O*D-----****X")) {
                    ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fontInfo);
                    x = bounds.x + bounds.width + bufferXR;
                    y = bounds.y + (labelHeight - descent) * 2;
                }else if (basicID===("G*G*GL5A--****X")) {//20200219
                    ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fontInfo,"middle");
                    x = bounds.x + (bounds.width * 0.5);
                    if(!byLabelHeight)
                    {
            ...
            if(modifiers.T)//T
            {
                ...
                if (basicID===("G*G*GPRE--****X")) {
                    ...
                    y = bounds.y + y;
                }else if (basicID===("G*O*D-----****X")) {
                    ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fontInfo,"end");

                    x = bounds.x - bufferXL;
                    y = ((labelHeight + bufferText) * 2);
                    y = bounds.y + y;
                }else if (basicID===("G*G*GL5A--****X")) {//20200220
                    ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fontInfo,"middle");
                    // points
                    x = bounds.x + (bounds.width * 0.5);
                    ...
                    basicID===("G*G*GPRE--****X")))//Node Signal Unit
            {
                ...
                //points
                x = bounds.x + (bounds.width * 0.5);
                if (basicID===("G*G*GPRE--****X") || basicID===("G*G*GL5A--****X")) {
            ...
            if((modifiers.X) &&//X
                    (basicID===("G*M*OHTL--****X") || basicID===("G*M*OHTH--****X") ) )//Low, High
            {
                strText = modifiers[ModifiersTG.X_ALTITUDE_DEPTH];
                ti = new SVGTextInfo(strText,new armyc2.c2sd.renderer.so.Point(0,0),fontInfo,"start");
    ...
    renderTextElement: function(tiArray, color, backgroundColor)
    {
        ...
        if ( size > 0 && tiArray[0].basicID != "WOS-HDS---P----" ) {
            if(backgroundColor)
            {
