...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.GetCenterLabel = function(tg) {
    ...
    try {
        switch (tg.get_LineType()) {
            case 22121000: // 작전활동부호 > 지휘통제 및 일반작전 > 일반 > 선 > 경계
                // 20201006
                var val = tg.b;
                if (val === 'C') {
                    label = '● ●';
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddBoundaryModifiers = function(tg, g2d, clipBounds, ptCenter) {//20200225 add "ptCenter"
    try {
        var j = 0;
        var csFactor = 1;
        ...
        */
                //20200225 echelonText add, AddIntegralModifier -> AddIntegralAreaModifier
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 2, TLineFactor * csFactor, ptCenter, ptCenter, new Boolean(true));
                if (!tg.get_EchelonSymbol().equals("")) {
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_EchelonSymbol(), 2, -0.2 * csFactor, ptCenter, ptCenter, new Boolean(true));
                }
                if (!tg.get_B().isEmpty()) { //20200225
                    var echelonText = SymbolUtilities.getEchelonText(tg.get_B());
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, echelonText, 2, -0.2 * csFactor, ptCenter, ptCenter, new Boolean(false));
                }
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_T1(), 2, T1LineFactor * csFactor, ptCenter, ptCenter, new Boolean(true));
      ...
      */
            ...
            armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 2, TLineFactor * csFactor, ptCenter, ptCenter, new Boolean(true));
            if (!tg.get_EchelonSymbol().equals("")) {
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_EchelonSymbol(), 2, -0.202 * csFactor, ptCenter, ptCenter, new Boolean(true));
            }
            if (!tg.get_B().isEmpty()) { //20200225
                var echelonText = SymbolUtilities.getEchelonText(tg.get_B());
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, echelonText, 2, -0.202 * csFactor, ptCenter, ptCenter, new Boolean(true));
            }
            armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_T1(), 2, T1LineFactor * csFactor, ptCenter, ptCenter, new Boolean(true));
            if (affiliation !== null && affiliation.equals("H")) {
                ptLast = armyc2.c2sd.JavaLineArray.lineutility.MidPointDouble(pt0, pt1, 0);
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddBoundaryModifiers2 = function(tg, g2d, clipBounds, ptCenter) {//20200225 add "ptCenter"
    try {
        var j = 0;
        var csFactor = 1;
        var foundSegment = new Boolean(false);
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifiersGeo = function(tg, g2d, clipBounds, converter) {
    try {
        ...
        console.log(tg.get_LineType());
        // 20201014 텍스트 위치조정 수정
        if (pts[0].x === pts[pts.length - 1].x && pts[0].y === pts[pts.length - 1].y) {
            ...
            // line
            var lineStringVals = [];
            for (var i in tg.LatLongs.array) {
                var pt = tg.LatLongs.array[i];
                lineStringVals.push([pt.x, pt.y]);
            }

            var line = turf.lineString(lineStringVals);
            var length = turf.length(line, {units: 'miles'});
            var along = turf.along(line, length / 2, {units: 'miles'});

            ptCenter = new armyc2.c2sd.JavaLineArray.POINT2(along.geometry.coordinates[0], along.geometry.coordinates[1]);
        }
        // 20201014 텍스트 위치조정 수정
        ...
        switch (linetype) {
            ...
            case 212240000:
            case 212220000:
                stringHeight = parseInt(0.5 * metrics.stringHeight());
        ...
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, tg.get_DTG() + " -", 2, 1 * factor * csFactor, middleSegment, middleSegment + 1, new Boolean(false));
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, tg.get_DTG1(), 2, 2 * factor * csFactor, middleSegment, middleSegment + 1, new Boolean(false)); */
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 2, -0.6, ptCenter, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG() + " -", 2, 1, pt0, pt1, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG1(), 2, 2, pt0, pt1, new Boolean(false));
                break;
            case 25224000:
                stringWidth = Math.floor((1.5 * metrics.stringWidth("ALT")));
                stringWidth2 = Math.floor((1.5 * metrics.stringWidth(label + tg.get_Name())));
            ...
            case 221231500:
            case 221231600:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name() + " " + tg.get_H() + " (" + tg.get_C() + ") " + tg.get_Name(), 2, -2, ptCenter, ptCenter, new Boolean(false));
                break;
            case 221231710:
            case 221231720:
                if (!tg.get_Name().isEmpty()) {
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + " " + tg.get_Name(), 2, -1.2, ptCenter, ptCenter, new Boolean(false));
                }else{
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 2, -1.2, ptCenter, ptCenter, new Boolean(false));
                }
                break;
            case 23163000:
                if (tg.Pixels.get(1).y > tg.Pixels.get(0).y) {
            ...
            case 22622000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.GetMBR(tg, ul, ur, lr, ll);
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 2, 1.1, ll, lr, new Boolean(false));
                if (!tg.get_Name().isEmpty()) {
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "(PL " + tg.get_Name() + ")", 2, 1.1 * (factor * csFactor + csFactor), ll, lr, new Boolean(false));
                }
                break;
            ...
            case 22522100:
                ...
                break;
            case 25226000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 2, -2, ptCenter, ptCenter, new Boolean(false));
                break;
            case 22535000:
            case 22432000:
            case 22621000:
            ...
            case 24322100:
                ...
                /* armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MIN ALT: " + tg.get_H(), 3, -1 * csFactor, ptCenter, ptCenter, new Boolean(false), "H");
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MAX ALT: " + tg.get_H1(), 3, 0, ptCenter, ptCenter, new Boolean(false), "H1"); */
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MIN ALT: " + tg.get_X(), 3, -1 * csFactor, ptCenter, ptCenter, new Boolean(false), "X");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MAX ALT: " + tg.get_X1(), 3, 0, ptCenter, ptCenter, new Boolean(false), "X1");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "Grids: " + tg.get_H2(), 3, 1 * csFactor, ptCenter, ptCenter, new Boolean(false), "H2");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "EFF: " + tg.get_DTG() + " -", 3, 2 * csFactor, ptCenter, ptCenter, new Boolean(false), "W");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG1(), 3, 3 * csFactor, ptCenter, ptCenter, new Boolean(false), "W1");
                break;
            ...
            case 243214100:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, 0, ptCenter, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG(), 3, 1 * csFactor, ptCenter, ptCenter, new Boolean(false));
                break;
            case 24361000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, -1 * csFactor, ptCenter, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 3, 0, ptCenter, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG() + dash + tg.get_DTG1(), 3, 1 * csFactor, ptCenter, ptCenter, new Boolean(false), "W+W1");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_X(), 3, 2 * csFactor, ptCenter, ptCenter, new Boolean(false));
                break;
            case 24324100:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, -1 * csFactor, ptCenter, ptCenter, new Boolean(true));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 3, 0, ptCenter, ptCenter, new Boolean(true));
            ...
            case 22431000:
                ...
                if (!tg.get_B().isEmpty()) { //20200225
                    var echelonText = SymbolUtilities.getEchelonText(tg.get_B());
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, echelonText, 2, -0.2 * csFactor, 0, 1, new Boolean(false));
                }
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.areasWithENY(tg, g2d);
                if (tg.get_N() !== null && tg.get_N() !== "") {
                    pt1 = armyc2.c2sd.JavaLineArray.lineutility.MidPointDouble(pt0, pt1, 0);
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_N(), 2, 0, pt0, pt1, new Boolean(true));
                    if (middleSegment !== 0) {
                        pt0 = tg.Pixels.get(middleSegment);
            ...
            case 22531000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "ASLT", 3, -1 * csFactor, ptCenter, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "PSN", 3, 0, ptCenter, ptCenter, new Boolean(false));
                //armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "\ub3cc\uaca9", 3, 0, ptCenter, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 3, 1 * csFactor, ptCenter, ptCenter, new Boolean(false));
                break;
            case 22235000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, -1.5 * csFactor, ptCenter, ptCenter, new Boolean(true));
            ...
            case 22234200:
                ...
                /* armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MIN ALT: " + tg.get_H(), 3, -0.5, ptCenter, ptCenter, new Boolean(false), "H");
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MAX ALT: " + tg.get_H1(), 3, 0.5, ptCenter, ptCenter, new Boolean(false), "H1"); */
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MIN ALT: " + tg.get_X(), 3, -0.5, ptCenter, ptCenter, new Boolean(false), "X");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MAX ALT: " + tg.get_X1(), 3, 0.5, ptCenter, ptCenter, new Boolean(false), "X1");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "TIME FROM: " + tg.get_DTG(), 3, 1.5, ptCenter, ptCenter, new Boolean(false), "W");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "TIME TO: " + tg.get_DTG1(), 3, 2.5, ptCenter, ptCenter, new Boolean(false), "W1");
                break;
            case 22223000:
          ...
          armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_H2(), 2, -6 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false)); */
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Max Alt: " + tg.get_X1(), 2, -4 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Min Alt: " + tg.get_X(), 2, -5 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_AM(), 2, -6 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false));
          ...
          armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_H2(), 2, -4 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false)); */
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Max Alt: " + tg.get_X1(), 2, -2 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Min Alt: " + tg.get_X(), 2, -3 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_AM(), 2, -4 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false));
          ...
          armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_H2(), 2, -6 * csFactor - dist, middleSegment, middleSegment + 1, new Boolean(false)); */
            ...
            case 22225000:
                if (tg.getSymbologyStandard() === 1) {
                    ...
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "Max Alt: " + tg.get_X1(), 2, -4 * csFactor, pt0, pt1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "Min Alt: " + tg.get_X(), 2, -5 * csFactor, pt0, pt1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "Width: " + tg.get_AM(), 2, -6 * csFactor, pt0, pt1, new Boolean(false));
            ...
            case 22138000:
                ...
                break;
            case 221310000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_H(), 3, 0, ptCenter, ptCenter, new Boolean(false));
                break;
            case 22121000:
                // 20201114 수정
                var lineStringVals = [];
                for (var a in tg.LatLongs.array) {
                    var pt = tg.LatLongs.array[a];
                    lineStringVals.push([pt.x, pt.y]);
                }
                var line = turf.lineString(lineStringVals);
                var length = turf.length(line, {units: 'miles'});

                var along = turf.along(line, length * 1/2, {units: 'miles'});
                var alongPoint = turf.getCoord(along);
                var alongPt = new armyc2.c2sd.graphics2d.Point2D(alongPoint[0],alongPoint[1]);
                var alongPixelPt = converter.GeoToPixels(alongPt)
                // B : 부대단위 symbol
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 2, -0.25, alongPixelPt, alongPixelPt, true);
                // T : 고유명칭
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_T(), 2, -1.75, alongPixelPt, alongPixelPt, true);
                // T1 : 고유명칭1
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_T1(), 2, 1.5, alongPixelPt, alongPixelPt, true);

                along = turf.along(line, length * 1/4, {units: 'miles'});
                alongPoint = turf.getCoord(along);
                alongPt = new armyc2.c2sd.graphics2d.Point2D(alongPoint[0],alongPoint[1]);
                alongPixelPt = converter.GeoToPixels(alongPt)
                // N : 적군표시 (LEFT)
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_N(), 2, -0.25, alongPixelPt, alongPixelPt, true);
                along = turf.along(line, length * 3/4, {units: 'miles'});
                alongPoint = turf.getCoord(along);
                alongPt = new armyc2.c2sd.graphics2d.Point2D(alongPoint[0],alongPoint[1]);
                alongPixelPt = converter.GeoToPixels(alongPt)
                // N : 적군표시 (RIGHT)
            ...
            case 221229000:
            case 221230000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, T1LineFactor, pt0, pt1, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, T1LineFactor, ptLast, ptNextToLast, new Boolean(false));
                break;
            case 221231100:
            case 221231200:
                // armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_H(), 2, -0.9, pt0, pt1, new Boolean(false));
                // armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_H1(), 2, -0.9, ptLast, ptNextToLast, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_H1(), 2, -1, ptLast, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_H(), 2, -1, pt0, ptCenter, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 2, -0.3, ptCenter, ptCenter, new Boolean(false));
                break;
            case 221232000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + " " +tg.get_T1(), 1, T1LineFactor, pt0, pt1, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + " " +tg.get_T1(), 1, T1LineFactor, ptLast, ptNextToLast, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "\uc8fc\ubbfc\uc774\ub3d9\ub85c " + tg.get_Name(), 2, -2, ptCenter, ptCenter, new Boolean(false));
                break;
            case 22423000:
            case 22424000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + " " +tg.get_Name(), 1, -1.2, pt0, pt1, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + " " +tg.get_Name(), 1, -1.2, ptLast, ptNextToLast, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG(), 1, 1.2, pt0, pt1, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG(), 1, 1.2, ptLast, ptNextToLast, new Boolean(false));
            ...
            case 22526000:
                //case 22613000:
                if (!tg.get_Name().isEmpty())
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "(PL " + tg.get_Name() + ")", 1, 1.3, pt0, pt1, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, 0, pt0, pt1, new Boolean(false));
                if (!tg.get_Name().isEmpty())
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "(PL " + tg.get_Name() + ")", 1, 1.3, ptLast, ptNextToLast, new Boolean(false));
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, 0, ptLast, ptNextToLast, new Boolean(false));
                break;
            case 22527000:
                if (!tg.get_Name().isEmpty())
        ...
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, -csFactor / 2, ptLast, ptNextToLast, new Boolean(false)); */
                if (!tg.get_Name().isEmpty()) {
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + "\n(PL " + tg.get_Name() + ")", 1, 0, pt0, pt1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + "\n(PL " + tg.get_Name() + ")", 1, 0, ptLast, ptNextToLast, new Boolean(false));
                }else{
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, 0, pt0, pt1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, 0, ptLast, ptNextToLast, new Boolean(false));
                }
        ...
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, -1, ptLast, ptNextToLast, new Boolean(false)); */
            ...
            //                break;
            case 22128000:
                /* armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 1, 0.9, pt0, pt1, new Boolean(false));
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "\ud574\uc548\uc120\n" + label , 1, -0.1, ptLast, ptNextToLast, new Boolean(false)); */
                pt0 = tg.Pixels.get(0);
                pt1 = tg.Pixels.get(1);
                pt2 = tg.Pixels.get(tg.Pixels.size() - 1);
                pt3 = tg.Pixels.get(tg.Pixels.size() - 2);
                ...
                if (tg.Pixels.size() === 2) {
                    ...
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, label, 2, 0.9 * csFactor, pt0, pt1, false);
                    if (dist > 2 * stringWidth + 5) {
                        pt0 = tg.Pixels.get(tg.Pixels.size() - 1);
                        pt1 = tg.Pixels.get(tg.Pixels.size() - 2);
                        if (pt0.x > pt1.x)
                            ...
                            pt1 = armyc2.c2sd.JavaLineArray.lineutility.ExtendAlongLineDouble(pt0, pt1, stringWidth); //was stringWidth
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, "\ud574\uc548\uc120\n" + label, 2, -0.1 * csFactor, pt0, pt1, false);
                    }
                } else {
                    var dist3 = armyc2.c2sd.JavaLineArray.lineutility.CalcDistanceDouble(pt0, pt2);
                    if (dist > stringWidth + 5 || dist >= dist2 || dist3 > stringWidth + 5) {
            ...
            case 24221000: //FSCL
                ...
                if (tg.Pixels.size() === 2) {
                    ...
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_Name() + " " + label, 2, -1.5 * csFactor, pt0, pt1, false);
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_DTG() + dash + tg.get_DTG1(), 2, 1.5 * csFactor, pt0, pt1, false);
                    if (dist > 2 * stringWidth + 5) {
                        pt0 = tg.Pixels.get(tg.Pixels.size() - 1);
                        pt1 = tg.Pixels.get(tg.Pixels.size() - 2);
                        if (pt0.x > pt1.x)
        ...
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, tg.get_DTG() + dash + tg.get_DTG1(), 2, 1, middleSegment, middleSegment + 1, new Boolean(false)); */
          ...
          armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_DTG(), 2, 0.7 * csFactor, pt0, pt1, false);
          armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_DTG1(), 2, 1.7 * csFactor, pt0, pt1, false); */
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + tg.get_T1(), 1, 0, pt0, pt1, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + tg.get_T1(), 1, 0, ptLast, ptNextToLast, new Boolean(false));
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, "RFL" + " " + tg.get_Name(), 2, -1.5 * csFactor, pt0, pt1, false);
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_DTG() + dash + tg.get_DTG1(), 2, 1.5 * csFactor, pt0, pt1, false);
                    if (dist > 2 * stringWidth + 5) {
                        pt0 = tg.Pixels.get(tg.Pixels.size() - 1);
                        pt1 = tg.Pixels.get(tg.Pixels.size() - 2);
            ...
            case 23350000:
                ...
                if (!tg.get_B().isEmpty()) { //20200225
                    var echelonText = SymbolUtilities.getEchelonText(tg.get_B());
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralModifier(tg, echelonText, 2, 0, 0, 1, new Boolean(true));
                }
                break;
            case 23410000:
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "1", 3, 0, pt1, pt1, new Boolean(true));
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.getRFALines = function(tg) {
    ...
    try {
        ...
        switch (tg.get_LineType()) {
            case 24352000:
            case 24363000:  //20200226 add
            case 24362000:
                /* if (tg.get_H1() !== null && !tg.get_H1().isEmpty())
          lines++; */
                //20200226 get_H1() -> get_X()
                if (tg.get_X() !== null && !tg.get_X().isEmpty())
                    lines++;
                break;
            default:
                break;
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.addSectorModifiers = function(tg, converter) {
    ...
    var H1 = tg.get_XN(); //20200226 H1->XN
    var T = tg.get_Name();
    var T1 = tg.get_T1();
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifiers2 = function(tg) {
    try {
        ...
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, "MAX ALT: " + tg.get_H1(), 2, 0, ptLeft, ptRight, false, "H1"); */
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, "MIN ALT: " + tg.get_X(), 2, -1 * csFactor, ptLeft, ptRight, false, "X");
                armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, "MAX ALT: " + tg.get_X1(), 2, 0, ptLeft, ptRight, false, "X1");
        ...
        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, "MAX ALT: " + tg.get_H1(), 3, 0, ptCenter, ptCenter, new Boolean(false), "H1"); */
            ...
            case 24363000:
                ...
                switch (rfaLines) {
                    case 4:
                        ...
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 3, 0, ptCenter, ptCenter, new Boolean(true));
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_DTG() + dash + tg.get_DTG1(), 3, 1 * csFactor, ptCenter, ptCenter, new Boolean(true), "W+W1");
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_X(), 3, 2 * csFactor, ptCenter, ptCenter, new Boolean(true), "H1");
                        break;
                    case 3:
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, -1 * csFactor, ptCenter, ptCenter, new Boolean(true));
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_Name(), 3, 0, ptCenter, ptCenter, new Boolean(true));
            ...
            case 21100000:
                ...
                break;
            case 21600000:
                // 견제 적군표시 로직 수정 20201210
                if (tg.get_N() !== '') {
                    for (j = 0; j < tg.Pixels.size(); j++) {
                        if (tg.Pixels.get(j).style === 14) {
                            pt0 = tg.Pixels.get(j);
                            pt1 = tg.Pixels.get(j + 1);
                            armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, tg.get_N(), 6, 0, pt0, pt1, new Boolean(true));
                            break;
                        }
                    }
                }
            ...
            case 22623001:
                ...
                //armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, 0, pt1, pt1, new Boolean(false));
                if (!tg.get_Name().isEmpty()) {
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label + "\n(PL " + tg.get_Name() + ")", 3, 0, pt1, pt1, new Boolean(false));
                }else{
                    armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddIntegralAreaModifier(tg, label, 3, 0, pt1, pt1, new Boolean(false));
                }
            ...
            case 24362000:
                ...
                switch (rfaLines) {
                    case 4:
                        ...
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_Name(), 2, 0, pt0, pt1, false);
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_DTG() + dash + tg.get_DTG1(), 2, 1 * csFactor, pt0, pt1, false, "W+W1");
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_X(), 2, 2 * csFactor, pt0, pt1, false, "H1");
                        break;
                    case 3:
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, label, 2, -1 * csFactor, pt0, pt1, false);
                        armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifier2(tg, tg.get_Name(), 2, 0, pt0, pt1, false);
          ...
          for (j = 0; j < H1.length; j++) {
            if (tg.Pixels.size() > j * 102 + 25) {
              ...
              armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddAreaModifier(tg, "ALT " + H1[j], 3, 0, pt0, pt0);
                ...
                //get_H1() -> get_XN(), H1 -> XN
                if (tg.get_XN() !== null && tg.get_XN().equals("") === false) {
                    XN = tg.get_XN().split(",");
                    for (j = 0; j < XN.length; j++) {
                        if (tg.Pixels.size() > j * 102 + 25) {
                            pt0 = tg.Pixels.get(j * 102 + 25);
                            armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddAreaModifier(tg, "ALT " + XN[j], 3, 0, pt0, pt0);
                        }
                    }
                }
                if (!tg.get_HideOptionalLabels()) {
                    /* var H2 = tg.get_H2(); */
                    var H2 = tg.get_AM();// get_H2() -> get_AM()
                    var am = H2.split(",");
            ...
            switch (modifier.type) {
                ...
                case 1: //to end
                    ...
                    if (pt0.x > pt1.x) {
                        ...
                        pt3 = armyc2.c2sd.JavaLineArray.lineutility.ExtendAlongLineDouble(pt3, pt2, 0 * stringWidth);
                    }else {
                        //pt3 = armyc2.c2sd.JavaLineArray.lineutility.ExtendAlongLineDouble(pt3, pt2, -stringWidth);
                        pt3 = armyc2.c2sd.JavaLineArray.lineutility.ExtendAlongLineDouble(pt3, pt2, -1 * stringWidth);
                    }
                    glyphPosition = new armyc2.c2sd.graphics2d.Point(pt3.x, pt3.y);
                    //diagnostic
                    //theta = 0;
...
armyc2.c2sd.JavaTacticalRenderer.Modifier2.AddModifiersGeo2 = function(tg,
    ...
    try {
        ...
        Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_H2(), aboveMiddle, -6 * csFactor, middleSegment, middleSegment + 1, false); */
                Modifier2.AddIntegralModifier(tg, "Max Alt: " + tg.get_X1(), aboveMiddle, -4 * csFactor, middleSegment, middleSegment + 1, false);
                Modifier2.AddIntegralModifier(tg, "Min Alt: " + tg.get_X(), aboveMiddle, -5 * csFactor, middleSegment, middleSegment + 1, false);
                Modifier2.AddIntegralModifier(tg, "Width: " + tg.get_AM(), aboveMiddle, -6 * csFactor, middleSegment, middleSegment + 1, false);
