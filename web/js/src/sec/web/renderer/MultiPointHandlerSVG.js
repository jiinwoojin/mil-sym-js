    ...
    return{
        ...
        GeoSVGize: function (symbolID, shapes, modifiers, ipc, normalize, format, hexTextColor, hexTextBackgroundColor, wasClipped, pixelWidth, pixelHeight, fillTexture, fontInfo, SVGFormat, converter)
        {
            ...
            for (var i = 0; i < modifiers.size(); i++) {
                ...
                if (modifier.getLineColor() !== null) {
                    alpha = modifier.getLineColor().getAlpha() / 255;
                }
            }
            ...
            if(paths && len > 0 && unionBounds)
            {
                ...
                {
                    var dom = new DOMParser().parseFromString(labels[j], 'text/html');
                    var body = dom.body;
                    var svg = '';
                    if (body) {
                        ...
                        for (var i = 0; i < texts.length; i++) {
                            var text = texts[i];
                            text.setAttribute('fill-opacity', alpha);
                            text.setAttribute('stroke-opacity', alpha);
                            svg += text.outerHTML;
                        }
                        group += svg + '\n';
                    }
                ...
                {
                    //return {svg:"data:image/svg+xml," + geoSVG, geoTL:geoCoordTL, geoBR:geoCoordBR, wasClipped:wasClipped, bounds:unionBounds};
                    return {svg: geoSVG, geoTL:geoCoordTL, geoBR:geoCoordBR, geoTR:geoCoordTR, geoBL:geoCoordBL, north:north, south:south, east:east, west:west, wasClipped:wasClipped, bounds:unionBounds};
                }
            }
            else
            {
