    ...
    return{
        ...
        ShouldClipSymbol: function (symbolID)
        {
            ...
            if (id === ("G*T*F-----****X") || id === ("G*F*LCC---****X") || id === ("G*G*GLB---****X") ||
                    ...
                    id === ("G*G*SAE---****X") || id === ("G*G*SAN---****X") || id === ("G*G*SAT---****X") || 
                    id === ("G*S*LRA---****X") || id === ("G*S*LRM---****X") ||
...
//            }
            ...
            try {
                ...
                var basicId = armyc2.c2sd.renderer.utilities.SymbolUtilities.getBasicSymbolID(symbolCode, 1);
                console.log(basicId);

                // [K] Maintain Angle About Alphabet
                // All Of Angles Default Will Be Calculated.
                ...
                // [K] Temporary Color Setting With Alpha
                if (symbolCode.charAt(0) === 'W' && shapes.size() > 0) {
                    var alpha = 255;
                    if (symbolModifiers[MilStdAttributes.LineColor]) {
                        var lc = armyc2.c2sd.renderer.utilities.Color.getColorFromHexString(symbolModifiers[MilStdAttributes.LineColor]);
                        alpha = lc.getAlpha();
                    }

                    for (var i = 0; i < shapes.size(); i++) {
                        var shape = shapes.get(i);
                        var lColor = shape.getLineColor();
                        var fColor = shape.getFillColor();

                        if (lColor) {
                            lColor = new armyc2.c2sd.renderer.utilities.Color(lColor.getRed(), lColor.getGreen(), lColor.getBlue(), alpha);
                            shape.setLineColor(lColor);
                        }
                        if (fColor) {
                            fColor = new armyc2.c2sd.renderer.utilities.Color(fColor.getRed(), fColor.getGreen(), fColor.getBlue(), alpha);
                ...
                {
                    var svgFormat = 1;
                    if (symbolModifiers[MilStdAttributes.SVGFormat] !== undefined)
...
//                }
        ...
        populateModifiers: function (modifiers, symbol)
        {
            ...
            try {
                ...
                if (modifiers[ModifiersTG.B_ECHELON])
                    modifierMap[ModifiersTG.B_ECHELON] = modifiers[ModifiersTG.B_ECHELON];
                ...
                if (modifiers[ModifiersTG.X_ALTITUDE_DEPTH])
                    modifierMap[ModifiersTG.X_ALTITUDE_DEPTH] = modifiers[ModifiersTG.X_ALTITUDE_DEPTH];
                
                if (modifiers[ModifiersTG.X1_ALTITUDE_DEPTH])
                    modifierMap[ModifiersTG.X1_ALTITUDE_DEPTH] = modifiers[ModifiersTG.X1_ALTITUDE_DEPTH];
                
                var i = 0;
                if (modifiers[ModifiersTG.XN_ALTITUDE_DEPTH] && modifiers[ModifiersTG.XN_ALTITUDE_DEPTH] instanceof Array)//20200226 X->XN
                {
                    var XN = modifiers[ModifiersTG.XN_ALTITUDE_DEPTH];
                    altitudes = [];
            ...
            try {
                ...
                else
                {
                    ...
                    if ( symbol.getLineColor() !== null && lineColor !== "transparent") { //20200313 
                ...
                {
                    if ( textColor !== "transparent") { //20200313 
                ...
                if (altitudes !== null) {
                    symbol.setModifiers_AM_AN_X(ModifiersTG.XN_ALTITUDE_DEPTH, altitudes);
                }
                if (distances !== null) {
        ...
        LabelToGeoJSONString: function (shapeInfo, ipc, normalize, textColor, textBackgroundColor)
        {
            ...
            if (text !== null && text !== ("")) {
                ...
                alpha = shapeInfo.getFillColor() && shapeInfo.getFillColor().getAlpha();
                if (alpha >= 0) {
                    feature.properties.fontOpacity = alpha / 255;
                }
